...
export function runProviderWorker(provider, upload, { timeoutMs = budgets.workerMs, launch = fork, key, traceRequestId } = {}) {
  return new Promise(resolveResult => {
    ...
    const child = launch(fileURLToPath(new URL('../scripts/provider-worker.mjs', import.meta.url)),
      [provider, upload.filePath], { stdio: ['ignore', 'ignore', 'ignore', 'ipc'], execArgv: [],
        ...(key === undefined && traceRequestId === undefined ? {} : { env: { ...process.env,
          ...(key === undefined ? {} : { REALITY_DEFENDER_API_KEY: key }),
          ...(traceRequestId === undefined ? {} : { REALCHECK_TRACE_REQUEST_ID: traceRequestId }) } }) });
