...
export function validateMetadata(name, size, mime = '') {
  ...
  if (!format) throw new ValidationError('Choose JPG, PNG, WebP, MP3, WAV, or FLAC. Video is not supported.');
  ...
  const aliases = { wav: ['audio/x-wav', 'audio/wave', 'audio/vnd.wave'], flac: ['audio/x-flac'], mp3: ['audio/mp3'] };
...
export async function withTemporaryUpload(stream, metadata, action, { tempRoot = tmpdir(), timeoutMs = 30000 } = {}) {
  const directory = await mkdtemp(join(tempRoot, 'realcheck-upload-'));
  const filePath = join(directory, 'media.' + metadata.extension);
  let handle, bytes, total = 0;
  const timer = setTimeout(() => stream.destroy(new ValidationError('Upload timed out. Please try again.')), timeoutMs);
  try {
