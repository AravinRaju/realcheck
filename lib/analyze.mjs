import { detectManipulation, transcribeAudio, providerFailure, ProviderError, logProviderFailure } from './providers.mjs';
...
export async function analyze(upload, { mode, keys = {}, detectionFixture = 'unclear', transcriptFixture = 'warning' },
  { detect = detectManipulation, transcribe = transcribeAudio, fixtureDelay = 800, providerLogger = console.warn } = {}) {
  ...
  if (mode === 'fixture') {
    ...
    if (authenticity.status === 'unavailable') logProviderFailure('reality_defender',
      detectionResult.status === 'rejected' ? detectionResult.reason : new ProviderError('invalid_response', { stage: 'response_validation' }), providerLogger);
    if (transcription.status === 'unavailable') logProviderFailure('groq',
      transcriptResult.status === 'rejected' ? transcriptResult.reason : new ProviderError('invalid_response', { stage: 'response_validation' }), providerLogger);
  }
