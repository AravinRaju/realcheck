...
import { sanitizedRealityDefenderStatus } from './rd-result.mjs';
...
export class ProviderError extends Error {
  constructor(code, details = {}) {
    ...
    this.timeoutCode = details.timeoutCode ?? null;
    this.providerStatus = sanitizedRealityDefenderStatus(details.providerStatus);
  }
}
export const SERVICE_LIMIT_MESSAGE = 'Service limit reached; try again later.';
const failureCodes = new Set(['internal', 'missing_key', 'sdk_missing', 'invalid_response', 'processing',
  'unsupported_status', 'timeout', 'network', 'unauthorized', 'invalid_request', 'server_error',
  'invalid_file', 'file_too_large', 'upload_failed', 'not_found', 'unknown_error',
  'worker_start_failed', 'worker_failed', 'provider_error']);
const safeFailureCode = code => typeof code === 'string' && (failureCodes.has(code) || /^http_[45]\d{2}$/.test(code)) ? code : 'internal';

export function logProviderFailure(provider, error, logger = console.warn) {
  const known = error instanceof ProviderError ? error : {};
  const entry = {
    event: 'provider_failure',
    provider: ['groq', 'reality_defender'].includes(provider) ? provider : 'unknown',
    code: safeFailureCode(known.code),
    stage: ['configuration', 'request', 'response_headers', 'response_body', 'response_validation', 'cooldown', 'worker'].includes(known.stage) ? known.stage : 'unknown',
    httpStatus: Number.isInteger(known.httpStatus) && known.httpStatus >= 100 && known.httpStatus <= 599 ? known.httpStatus : null,
    timeoutCode: ['TimeoutError', 'AbortError', 'ETIMEDOUT', 'worker_timeout'].includes(known.timeoutCode) ? known.timeoutCode : null,
    networkCode: ['EACCES', 'EPERM', 'ECONNREFUSED', 'ENOTFOUND', 'ETIMEDOUT', 'ECONNRESET'].includes(known.networkCode) ? known.networkCode : null,
  };
  if (provider === 'reality_defender' && entry.code === 'unsupported_status') {
    entry.providerStatus = sanitizedRealityDefenderStatus(known.providerStatus);
  }
  // One allowlisted record, never an Error object, body, upload or transcript.
  // Logging failure must not change either independent provider result.
  try { logger(JSON.stringify(entry)); } catch { /* Preserve analysis results. */ }
}
...
export function providerFailure(error) {
  const result = { status: 'unavailable', label: 'Analysis unavailable', code: error instanceof ProviderError ? safeFailureCode(error.code) : 'internal' };
  if (result.code === 'unsupported_status') {
    const status = sanitizedRealityDefenderStatus(error.providerStatus);
    if (status) result.providerStatus = status;
  }
...
export async function detectManipulation(upload, key, { worker = runProviderWorker } = {}) {
  if (!key) throw new ProviderError('missing_key', { stage: 'configuration' });
  if (typeof upload?.filePath !== 'string' || !upload.filePath) throw new ProviderError('invalid_file', { stage: 'request' });
  ...
  throw new ProviderError(allowed.has(result?.error) ? result.error : 'provider_error', {
    stage: 'worker', timeoutCode: result?.error === 'timeout' ? 'worker_timeout' : null,
    providerStatus: result?.outcome === 'response_received' && result?.error === 'unsupported_status' ? result.providerStatus : null,
  });
}
...
export function createTranscriber({ now = Date.now } = {}) {
  ...
  return async function transcribeAudio(upload, key, { fetcher = fetch, timeoutMs = budgets.providerMs } = {}) {
    if (!key) throw new ProviderError('missing_key', { stage: 'configuration' });
    if (now() < retryAt) throw new ProviderError('http_429', { stage: 'cooldown', retryAfterSeconds: Math.ceil((retryAt - now()) / 1000) });
    ...
    try {
      ...
      throw new ProviderError(error.name === 'TimeoutError' || error.name === 'AbortError' ? 'timeout' : 'network', {
        stage: 'request', networkCode,
        timeoutCode: ['TimeoutError', 'AbortError'].includes(error.name) ? error.name : networkCode === 'ETIMEDOUT' ? networkCode : null,
      });
    }
    if (!response.ok) {
      ...
      throw new ProviderError('http_' + response.status, { stage: 'response_headers', httpStatus: response.status, retryAfterSeconds });
    }
    let body;
    try { body = await response.json(); } catch (error) {
      throw new ProviderError(['TimeoutError','AbortError'].includes(error.name) ? 'timeout' : 'invalid_response', {
        stage: 'response_body', httpStatus: response.status,
        timeoutCode: ['TimeoutError','AbortError'].includes(error.name) ? error.name : null,
      });
    }
    const transcript = parseTranscript(body);
    if (!transcript) throw new ProviderError('invalid_response', { stage: 'response_validation', httpStatus: response.status });
